...
graphs['u_counter'] = Counter('python_request_u_posts', 'The total number'\
  + ' of card use posts.')
 ##PART 3 - adding logging for error messages 
graphs['error_return_counter'] = Counter('python_return_error', 'The total number'\
  + ' of errors returned')
...
def register_view(request):
    if request.method == 'GET':
        ...
        #PART 3 - FIX: commenting this out as this may reveal secrets during login process in debug logs
        ## recording the password in counter is a huge issue and is remediated by commenting out this entirely
...
def login_view(request):
    if request.method == "GET":
        ...
        #PART 3 FIX: graphs['l_counter'].inc() we don't want to expose number of logins 
        context = {'method':'POST'}
        uname = request.POST.get('uname', None)
        pword = request.POST.get('pword', None)
